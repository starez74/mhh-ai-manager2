...
type Quote={id:string;created_at:string;quote_number:string;status:string;enquiry_id?:string;customer_id?:string;customer_name:string;phone:string;email:string;pickup_suburb:string;delivery_suburb:string;preferred_date:string;scope_summary:string;risk_flags:string;missing_information:string;draft_message:string;price_amount?:number;deposit_amount?:number;valid_until?:string;internal_notes:string};
type Job={id:string;created_at:string;job_number:string;status:string;quote_id?:string;enquiry_id?:string;customer_id?:string;customer_name:string;phone:string;email:string;scheduled_start?:string;scheduled_end?:string;pickup_address:string;delivery_address:string;pickup_suburb:string;delivery_suburb:string;crew:string;vehicle:string;scope_summary:string;special_instructions:string;quoted_amount?:number;paid_amount:number};
...
export default function Dashboard(){
 ...
 const openQuotes=quotes.filter(q=>['draft','approved','sent'].includes(q.status)).length;
 const upcomingJobs=jobs.filter(j=>j.scheduled_start&&new Date(j.scheduled_start)>=new Date()&&!['completed','cancelled'].includes(j.status)).length;
 ...
 const filteredQuotes=useMemo(()=>quotes.filter(q=>(q.quote_number+' '+q.customer_name+' '+q.pickup_suburb+' '+q.delivery_suburb+' '+q.status).toLowerCase().includes(search.toLowerCase())),[quotes,search]);
 const filteredJobs=useMemo(()=>jobs.filter(j=>(j.job_number+' '+j.customer_name+' '+j.pickup_suburb+' '+j.delivery_suburb+' '+j.status).toLowerCase().includes(search.toLowerCase())),[jobs,search]);
 ...
 return <div className="shell">
  ...
  <section className={view==='settings'?'view active':'view'}><div className="card"><h3>System</h3><p>Application version: <strong>5.0.0</strong></p><p>Operational tables: <strong className="success">enquiries + customers + quotes + jobs + activity</strong></p><p>Quote AI: <strong className="success">Owner-controlled pricing</strong></p><div className="notice">Run supabase/v5-migration.sql before opening Quotes or Jobs.</div><button className="btn danger" onClick={signOut}>Sign out</button></div></section>
  </main>
 </div>
}
