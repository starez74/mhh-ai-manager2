...
type View="dashboard"|"create"|"campaigns"|"calendar"|"settings";
...
export default function Dashboard(){
 ...
 {([["dashboard","Dashboard"],["create","Create Campaign"],["campaigns","Campaign Library"],["calendar","Content Plan"],["settings","Settings"]] as [View,string][]).map(([id,label])=><button key={id} className={view===id?"active":""} onClick={()=>setView(id)}>{label}</button>)}
 </div></aside>
 ...
 <section className={view==="settings"?"view active":"view"}><div className="card"><h3>Security & Connections</h3><p>Signed in with Supabase authentication.</p><p>Campaigns are protected by Row Level Security and belong only to the signed-in account.</p><p>Facebook publishing: <strong className="error">Not connected</strong></p><p>GitHub website editing: <strong className="error">Not connected</strong></p><div className="notice">Do not add Facebook or GitHub passwords to Vercel. Those connections must use OAuth or restricted tokens.</div><button className="btn danger" onClick={signOut}>Sign out</button></div></section>
 </main></div>;
}
